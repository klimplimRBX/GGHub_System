return function(ctx)
	...
	createDropdown(settingsPage, "Tema",
		{"Default", "Crimson", "Evergreen", "Yellow", "Navy Blue", "Cosmic Purple", "Pink Flakes", "True Black", "Amber", "Custom"},
		function(value)
			if value == "Custom" then
				openColorPicker(ctx.theme.customH, ctx.theme.customS, ctx.theme.customV)
				return
			end
			local nameMap = {
				["Default"]       = "Default",
	...
	createDropdown(settingsPage, "Tamanho do Botão GG",
	...
	local function setSliderVisible(visible, animate)
		...
		if animate then
			if visible then glowSliderWrapper.Visible = true end
			TweenService:Create(glowSliderWrapper,
				TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
				{Size = UDim2.new(1, 0, 0, targetH)}
			):Play()
			if not visible then
				task.delay(0.26, function() glowSliderWrapper.Visible = false end)
			end
		else
	...
	createToggle(settingsPage, "Glow Effect", "Glow ao redor da janela principal", function(state)
		glowEnabled = state
	...
	createSlider(glowSliderWrapper, "Intensidade do Glow", "Brilho do efeito glow", 0, 100, glowIntensity * 100, function(val, pct)
		glowIntensity = pct
	...
	KBDesc.Text = "Abrir/fechar o hub no PC"
	...
	KBBtn.MouseButton1Click:Connect(function()
		...
		_bindingMode = true
		KBBtn.Text = "Pressione uma tecla..."
	...
	createButton(settingsPage, "Salvar Configurações", "Salva todos os toggles e preferências", function()
		saveToggleStates()
		showNotification("Configurações salvas!")
	end)

	createButton(settingsPage, "Resetar Configurações", "Limpa todos os estados de toggle salvos", function()
	...
	local function loadOldScript(url)
		...
		local targetGui = playerGui:FindFirstChild("GGHub_GAG") or playerGui:FindFirstChild("GGHub_v09")
	...
	createButton(settingsPage, "Easter Event Script", "Vai para a versão do GGHub do evento de Easter", function()
		loadOldScript("https://raw.githubusercontent.com/klimplimRBX/GGHub/main/GGHubEasterEvent.lua")
	end)
end
